% LINFFITTING
% Test script for nonlinear L^\infty data fitting using primal-dual
% extragradient methods. Solves
%   min_u 1/2 |u|_L2^2 s.t. |S(u)-yd|\leq \delta a.e. in \Omega,
...
% noisy data (quantization noise)
nbin  = 11;                  % number of bins (~ magnitude of noise)
n     = nel+1;               % number of nodes
ys    = (max(ye)-min(ye))/(nbin-1);
yd    = ys*round(ye/ys);     % quantized data
delta = norm(yd-ye,inf);     % compute exact noise level

% show noisy data
figure(1), plot(x,ye,x,yd),  title('Data'),  legend('exact','noisy');

% regularized objective functional (pointwise)
f_gamma = @(y) 1/(2*gamma)*max(0,abs(y)-delta).^2;
...
for it = 1:maxit
    ...
    yb = fem.A(ub)\Mf;
    q  = p + sigma*(yb-yd);
    p  = 1/(1+sigma*gamma)*max(abs(q)-delta*sigma,0).*sign(q);
